    ...
    <div className="container mx-auto bg-slate-600 max-w-none">
      ...
      >
        ...
        <div className="z-20 flex flex-col gap-y-8 text-white">
          ...
          <p className="text-xl xl:text-2xl font-light">{movie.overview}</p>

            {movie.genres && (
              <div className="flex items-center justify-center gap-x-3">
                {movie.genres.map((genre, index) => (
                  <div key={index} className="border border-white rounded-lg px-4 py-1 cursor-pointer hover:bg-slate-50 hover:text-slate-600 duration-300">{genre.name}</div>
                ))}
              </div>
            )}
        </div>
